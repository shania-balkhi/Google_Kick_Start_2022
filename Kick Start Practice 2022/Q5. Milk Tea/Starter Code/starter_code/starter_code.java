...
public class Solution {
  ...
  static int findSmallestNumberOfComplaints(int[][] friendsOrders, int[][] forbiddenOrders) {
    ...
    int[][] notForbiddenOrders = friendsOrders;
    for(int i = 0; i < friendsOrders.length; ++i){
      for(int j = 0; j < forbiddenOrders.length; ++j){
        if (friendsOrders[i][j] == forbiddenOrders[i][j]){
          notForbiddenOrders[i][j] = notForbiddenOrders[i][j] - friendsOrders[i][j];
        }
      }
    }
    smallestNumberOfComplaints = notForbiddenOrders[i];
